...
- **Agentic sessions** — open-ended, streaming chat against your configured models, with your workspace context and tools from MCP servers you configure (e.g. web search via the Tavily MCP server). A layered system prompt — a workspace `kiri.md` plus optional `personas/` — shapes every session.
