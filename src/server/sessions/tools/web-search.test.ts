...
// be asserted. Returns whatever `body` is given.
const stubTavily = (body: Record<string, unknown>) => {
  let request: Request | undefined;
  server.use(
    http.post(TAVILY_URL, async ({ request: req }) => {
      request = req.clone();
...
describe("searchTavily", () => {
  ...
  it("treats a response with no results array as zero results", async () => {
    stubTavily({});
    const output = await searchTavily({ apiKey: "k", query: "q" });
    expect(output).toEqual({ query: "q", answer: undefined, results: [] });
  });
  ...
  it("throws without echoing the api key on a non-2xx response", async () => {
    ...
    expect(message).toBe("tavily search failed: 401 Unauthorized");
...
describe("web_search tool", () => {
  it("runs a search through its execute and returns the parsed output", async () => {
    stubTavily({ results: [{ title: "Result", url: "https://r.com", content: "snippet" }] });
