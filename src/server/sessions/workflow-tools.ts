...
export function workflowTools(deps: WorkflowToolsDeps): ToolSet {
  ...
  return {
    ...
    rerun_workflow: tool({
      description:
        "Re-execute a finished run of one of the workspace's workflows and wait for it to finish, replacing that run's previous results under the same run_id and activity-feed entry. Use it for authorized repeats, including safe test iterations within an authorized fix or test task. It re-executes the whole workflow, not just the failed step: failure or timeout does not prove earlier actions had no effects. Inspect what completed before repeating; do not blindly duplicate external effects such as sending, publishing, charging, or deleting. Ask before re-execution if prior effects or permission to repeat are unclear. The workflow's current definition is what runs, so edits made since the last run apply. Inputs follow run_workflow's contract and are not carried over from the previous run — re-supply every required input. Returns the same outcome shape as run_workflow.",
      inputSchema: z.object({
        run_id: z
          .string()
          .min(1)
    ...
    create_workflow: tool({
      description:
        "Create a new workflow in this workspace: a repeatable automation saved as a YAML file in workflows/, which kiri runs on demand. Use it only when the user explicitly requests creating a workflow. A repeated task, a wish to keep a result, or an unanswered suggestion is not authorization to create one. When available, load the workflow-authoring skill with use_skill before authoring; reuse it while its instructions are available and current. It describes the YAML contract under the applicable instruction precedence. Supply the complete YAML file content; it is validated before anything is written, and a validation rejection names what to fix (YAML parse error, schema violation, unknown bundle or llm provider, missing prompt file). Correct validation errors and retry within the authoring task; never retry or bypass a permission denial. The workflow appears in the catalog immediately. Saving it does not authorize execution.",
      inputSchema: z.object({
