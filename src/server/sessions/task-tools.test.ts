...
describe("taskTools", () => {
  ...
  it("summarises a project's list as visible group and open counts", () => {
    expect(summariseTaskList(db, "p1")).toEqual({ groups: 2, open: 2 });
    updateTaskGroup(db, "g1", { hidden: true });
    expect(summariseTaskList(db, "p1")).toEqual({ groups: 1, open: 0 });
    expect(summariseTaskList(db, "missing")).toEqual({ groups: 0, open: 0 });
  });

  it("never lists hidden groups", async () => {
    updateTaskGroup(db, "g2", { hidden: true });
    const output = (await run(tools.list_tasks, {})) as { groups: { name: string }[] };
    expect(output.groups.map((group) => group.name)).toEqual(["Now"]);
  });

  it("hides a group through update_task_group, after which it is out of reach", async () => {
    expect(await run(tools.update_task_group, { name: "Now", hidden: true })).toEqual({
      group: "Now",
      hidden: true,
      updated: true,
    });
    expect(listTaskGroups(db, "p1").find((group) => group.id === "g1")?.hidden).toBe(true);
    // Its name, its tasks, and a name clash with it all resolve as hidden.
    await expect(run(tools.update_task_group, { name: "Now", new_name: "X" })).rejects.toThrow(
      "is hidden",
    );
    await expect(run(tools.update_task, { id: "t1", done: true })).rejects.toThrow("hidden group");
    await expect(run(tools.add_task, { group: "Now", title: "x" })).rejects.toThrow("is hidden");
    createTask(db, "g2", { title: "three" }, { id: "t3" });
    await expect(run(tools.update_task, { id: "t3", group: "Now" })).rejects.toThrow("is hidden");
    await expect(run(tools.create_task_group, { name: "Now" })).rejects.toThrow("is hidden");
    await expect(run(tools.update_task_group, { name: "Later", new_name: "Now" })).rejects.toThrow(
      "is hidden",
    );
    await expect(run(tools.delete_task_group, { name: "Now" })).rejects.toThrow("is hidden");
  });
