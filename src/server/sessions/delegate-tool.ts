...
export function delegateTool(deps: DelegateToolDeps): ToolSet {
  ...
  return {
    [DELEGATE_TOOL_NAME]: tool({
      description:
        "Delegate a self-contained task to a separate worker session that runs it with its own tools and context and returns only a written report. Reach for it for multi-step research or gathering — anything needing several searches, fetches, or reads whose intermediate results you don't need to keep — so this conversation holds the findings, not the working. The worker cannot see this conversation: write the task as a complete brief, stating the goal, every specific it needs, and the shape of report you want back. The worker holds only tools the user allows to run without approval; it runs while you wait. The report is the delegated work, done — answer from it rather than re-running any of it yourself.",
