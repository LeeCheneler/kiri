...
import { z } from "zod";
import type * as searchApi from "../../shared/api/search.ts";
...
}) satisfies z.ZodType<searchApi.SearchQuery>;
...
export function searchRoutes(deps: SearchRoutesDeps): Hono {
  ...
  app.get("/", zValidator("query", searchQuerySchema, onZodFail("invalid search query")), (c) => {
    ...
    return c.json(search(deps, q, limit) satisfies searchApi.SearchResults);
  });
