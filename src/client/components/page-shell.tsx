...
export function PageShell({ children }: { children: ReactNode }) {
  return (
    <div className="mx-auto min-h-screen max-w-[1240px] px-8 py-12 lg:py-16">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[200px_1fr_260px] lg:gap-12">
        <aside className="lg:sticky lg:top-16 lg:self-start">
          <h1 className="leading-none">
            <Link
              href="/"
              className="font-display text-[34px] italic text-ink no-underline transition-colors duration-150 hover:text-accent"
            >
              kiri
            </Link>
          </h1>
        </aside>
        <main className="min-w-0 lg:max-w-[640px]">{children}</main>
