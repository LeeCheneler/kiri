...
/**
 * The workflow detail tab strip: the workflow's recent runs, its declared
 * inputs, what it publishes, and its pipeline schema. Built on the
 * design-system `Tabs`, so the active tab deep-links to the URL.
 */
export function WorkflowDetails({ workflow }: { workflow: WorkflowSummary }) {
  const tabs: TabDef[] = [
    { id: "recent", label: "Recent runs", content: <RecentRuns /> },
