...
Permissions are keyed by the tool's namespaced `<server>__<tool>` name — arguments are not part of the key — and persist in a gitignored **`.kiri/tool-permissions.json`**: only the non-default verdicts are stored (`allow` or `off`), an absent entry reading back as `ask`. The file is read on every tool request so a decision is honoured across sessions and restarts, and a hand-edit takes effect on the next turn with no restart. Unlike `mcp-credentials.json` it is plain JSON with default permissions — tool names are not secrets. The **MCP page** (`/mcp`) is the surface for reviewing and changing these: it lists each configured server, its connection state (with the same Connect prompt for an unsigned-in OAuth server), and its tools, each with an Always allow / Ask / Off control — the inline prompt's Always allow is just a shortcut that writes the same `allow`. One subtlety the SDK forces: it re-checks `needsApproval` when resuming and denies a call that no longer needs approval, so a tool flipped to `allow` mid-turn still reports as needing approval for a call the user has *already* answered this turn — the new permission takes effect from the next fresh call, not by cancelling the one in flight.

First-party tools that only read or write kiri's own data — the article tools and `list_workflows` — sit outside this machinery: they are offered on every turn un-gated (the request in chat is the authorisation, so an approval prompt would add friction without trust), merged after the gated MCP set so a name collision resolves to the first-party tool, and not listed on the MCP page. `run_workflow` is the exception among the built-ins: it executes user-authored scripts, so it rides the same standing permission as an MCP tool — `ask` by default, keyed by its plain (un-namespaced) name — and the MCP page lists it under a **Built-in tools** card so a permission recorded from the transcript's Always allow can be reviewed and reversed.
