...
- **Session chat (`/sessions/:id`).** The conversation transcript with a composer, and a right rail carrying the session's model — swappable mid-conversation, applying from the next turn — its running token totals, current context fill, and the session's actions: pin/unpin (onto the feed's Pinned tab) and delete.
