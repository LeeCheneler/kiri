...
- **`sessions`** — one row per conversation: chosen model, the attached persona (a `personas/<name>.md` name, or null for none), status (`running` | `idle` | `failed` | `cancelled`), timestamps, and its **lineage** — a nullable `parent_session_id` (with the spawning `parent_tool_call_id`) and a `kind` (`chat` | `investigation`) that together mark a child investigation spawned by a parent's `investigate` call (see *Investigations*).
...
24. **Investigate sub-agent.** A first-party `investigate` tool (see *Agentic sessions → Investigations*) that delegates a research task to a child **investigation** session: a client-completed tool whose call pauses the parent turn, runs the child against the same turn loop under a focused investigator prompt, and resumes the turn with only the child's report — so the parent's context holds the findings, not the intermediate searches. Sessions gain a `parent_session_id`/`kind` lineage; child investigations are hidden from the feed but reachable by URL, render inline as an embedded-transcript box (collapsed by default, opening for inner approvals), and the core tool guidance steers research to the tool over direct search.
