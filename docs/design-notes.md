...
Layers compose in order — **core → `kiri.md` → persona** — and all three are read fresh from disk each turn, so an edit takes effect on the next turn with git as the source of truth; nothing is snapshotted. A missing `kiri.md` and no persona is a first-class default: the session runs on the core layer alone, a plain chat. The one exception is a **child session** (see *Delegation*): identified by its parent link alone (there is no separate kind), it runs a focused, kiri-authored worker prompt in place of this stack — it is a delegated worker on a single task, not the user's chat, so `kiri.md` and personas don't apply. The worker prompt keeps the capability guidance keyed off the child's active tool set (the sandbox roots, the shell rules), so a worker holding those tools still gets their contracts.

This deliberately replaces an earlier `agents/*.yaml` registry idea (a pre-baked definition bundling system prompt, model, tools, and params, snapshotted onto the session). For a single-user tool, one always-on `kiri.md` plus opt-in personas covers role-switching without a registry or a per-session config snapshot. Personas stay plain files — nothing is loaded into a registry, and the only machinery around them is a watcher over `personas/` that publishes a `persona.changed` event so the picker follows a file being added, edited, or removed without a reload. The list itself is read from disk on demand. The model is chosen at session creation and swappable mid-conversation; the persona is likewise attached and swappable from the session's aside — a nullable `persona` column on the `sessions` row records the selection, the same posture as `model` — applying from the next turn.

### Storage

Two new tables alongside the existing four, following the runs/run_steps shape:

- **`sessions`** — one row per conversation: chosen model, the attached persona (a `personas/<name>.md` name, or null for none), status (`running` | `idle` | `failed` | `cancelled`), timestamps, a **running token total** (see *Usage* below), and its **lineage** — a nullable `parent_session_id` (with the spawning `parent_tool_call_id`) marking a child session spawned by a parent's `delegate` call (see *Delegation*). The parent link alone distinguishes a delegated worker from a top-level chat — there is no separate kind. Deleting a session cascades to its children.
