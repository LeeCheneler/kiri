...
The model is taught to author through **progressive disclosure** rather than the system prompt: a `read_workflow_authoring_guide` tool returns the full authoring reference (file shape, step kinds, data-flow and env rules, articles/summarize, working method — distilled from the workspace authoring reference, scoped to what a session can do), and each write tool's description says to load it once per conversation before authoring. Sessions that never author pay only the tool descriptions; when generic skill support lands, the guide content is the obvious first skill. Model selection is ask-first: the guide forbids inventing a `provider:model`, sending the model to the user's standing `kiri.md` preferences, then an existing workflow's choice, then a question to the user — and an unknown-provider rejection names the configured set. Out of scope, deliberately: authoring bundles or prompt template files (sessions have no tools for them; the guide steers to inline prompts and existing bundles), and deleting workflows.
