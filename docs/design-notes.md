...
The first-party article tools sit outside this machinery: they are offered on every turn un-gated — they write only to kiri's own database and the request in chat is the authorisation, so an approval prompt would add friction without trust — merged after the gated MCP set so a name collision resolves to the first-party tool, and they are not listed on the MCP page.
