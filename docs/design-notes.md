...
- **`sessions`** — one row per conversation: chosen model, status (`running` | `idle` | `failed` | `cancelled`), timestamps, a **running token total** (see *Usage* below), and its **lineage** — a nullable `parent_session_id` (with the spawning `parent_tool_call_id`) marking a child session spawned by a parent's `delegate` call (see *Delegation*). The parent link alone distinguishes a delegated worker from a top-level chat — there is no separate kind. Deleting a session cascades to its children.
