...
- **`sessions`** — one row per conversation: chosen model, the attached persona (a `personas/<name>.md` name, or null for none), status (`running` | `idle` | `failed` | `cancelled`), timestamps, and its **lineage** — a nullable `parent_session_id` (with the spawning `parent_tool_call_id`) marking a child session spawned by a parent's tool call, an investigation (see *Investigations*). The parent link alone distinguishes a delegated worker from a top-level chat — there is no separate kind.
...
Grants are keyed by the tool's namespaced `<server>__<tool>` name — arguments are not part of the key — and persist in a gitignored **`.kiri/tool-grants.json`**, read on every tool request so the server honours an Always-allow decision across sessions and restarts, and a hand-edit (deleting an entry to revoke a grant) takes effect on the next turn with no restart. Unlike `mcp-credentials.json` the file is plain JSON with default permissions — tool names are not secrets. There is no in-app surface to view or revoke grants; editing the file is the escape hatch. One subtlety the SDK forces: it re-checks `needsApproval` when resuming and denies a call that no longer needs approval, so a granted tool still reports as needing approval for a call the user has *already* answered this turn — the recorded grant takes effect from the next fresh call, not by cancelling the one in flight.

### Investigations

The one **first-party tool** is `investigate` — a way to delegate research without filling the conversation with it. It is offered only to a top-level session (never to a child sub-session, so a worker can't spawn another) and is *not* itself approval-gated; the trust gates are the inner tool calls it makes. It is a **client-completed** tool: it has no server `execute`, so the model's call pauses the turn with the call awaiting its output, and the client computes the result and supplies it to resume — the same pause/resume shape as a tool approval, reached through the AI SDK's `addToolOutput`.

The work runs as a **child session**. The box rendered for the call get-or-creates a real, full session — a child marked by a `parent_session_id` and the spawning `parent_tool_call_id` linking it to the turn — and drives it with the model's `task` as the first message, reusing the whole session turn loop: the investigator runs its own multi-step tool calls (typically the same web-search/fetch MCP servers the parent has), each inner call gated by the same per-call approval as any session tool. It runs under a focused, kiri-authored *investigator* system prompt rather than the core→`kiri.md`→persona stack, and cannot see the parent conversation — the `task` is its entire brief.

Only the investigator's final report crosses back, as the tool's output: the parent's context holds the findings, not the intermediate searches. That rollup is the whole point, and the core tool guidance steers the model to delegate multi-step research here rather than searching itself. A child investigation is **hidden from the feed and the session list** (those show only top-level `parent_session_id IS NULL` sessions) yet first-class — reachable at its own `/sessions/:id` URL and resumable like any session. Get-or-create is **idempotent on the spawning tool call**, so a reload re-attaches the running child rather than starting a duplicate; if the child fails or is cancelled the box reports a note so the parent never hangs. The transcript renders the call as a box — a reusable embedded child-session view, not specific to `investigate` — that shows the child's live transcript inline, collapsed to its task and status by default like any tool block, expanding to the worker's tool calls and report, and opening automatically while an inner tool awaits approval. The box titles itself from the spawning tool's name, so any client-completed tool that runs as a child session reuses it; `investigate` is the only one today.
...
Sessions are activity and belong in the feed. The home feed is a polymorphic union over `runs` + `sessions`, newest-first by `startedAt` with a composite `(startedAt, id)` cursor, fronted by an `All · Workflows · Sessions` tab strip that filters it to either kind. The filtered tabs reuse the per-kind `runs` and `sessions` feeds, so only the union carries the composite cursor. Child **investigation** sessions are excluded from every feed and the session list — all of them filter to top-level `parent_session_id IS NULL` rows — since an investigation is part of its parent's turn, not standalone activity; it surfaces inline in the parent and at its own URL.
...
<repo-root>/
  ...
  .kiri/                      # gitignored — repo-scoped runtime state
    ...
    tool-grants.json          # "Always allow" session tool grants (plain JSON; tool names aren't secrets)
```
...
- **Session tools approved per call.** In agentic sessions, a tool injected via prompt-injection still can't run silently: each call pauses for an explicit Allow / Always allow / Deny before it executes (see *Agentic sessions → Tool permissions*). Persisted Always-allow grants live in a gitignored `.kiri/tool-grants.json` — plain JSON, not a secret store — keyed by tool name and re-read on every call, so revoking is deleting an entry.
...
22. **Tool permission prompts.** Each tool call in an agentic session is gated per invocation: the turn pauses and the transcript shows an **Allow / Always allow / Deny** prompt with the call's input before it runs. Allow runs it once, Deny refuses it and tells the model so, Always allow runs it and persists a grant (keyed by `<server>__<tool>`) to a gitignored `.kiri/tool-grants.json` so that tool never prompts again, across sessions and restarts. Built on the AI SDK's native tool-approval flow — the request rides the existing turn stream and a verdict resubmits to resume — with grants read on every call so a hand-edit revokes them.
23. **Bounded, cancellable tool calls.** Every MCP tool call is wrapped so its result is capped at 128 KB (truncated with a marker past it) — a large payload can't overrun the model's context or the provider's request-size limit — and given a 180-second budget that rides the model's own cancel signal, after which the call is aborted and reported as a tool error the model can work around. A still-running call can be cancelled from its block in the transcript, and a cancelled call reads as cancelled rather than sitting on "working".
24. **Investigate sub-agent.** A first-party `investigate` tool (see *Agentic sessions → Investigations*) that delegates a research task to a child **investigation** session: a client-completed tool whose call pauses the parent turn, runs the child against the same turn loop under a focused investigator prompt, and resumes the turn with only the child's report — so the parent's context holds the findings, not the intermediate searches. Sessions gain a `parent_session_id` lineage (a child is marked by its parent alone, no separate kind); child investigations are hidden from the feed but reachable by URL, render inline as a reusable embedded child-session box (collapsed by default, opening for inner approvals), and the core tool guidance steers research to the tool over direct search.
