...
- Session filesystem access is opt-in and sandboxed: the filesystem tools exist only when `filesystem.allowed_directories` is declared in `kiri.yaml`, every model-supplied path must resolve — through symlinks — to inside a declared directory, and hidden (dot-prefixed) paths are unreachable.
...
- **Session file reads are sandboxed and opt-in.** The filesystem read tools exist only when `filesystem.allowed_directories` is declared in `kiri.yaml` — a git-reviewable boundary, like the `mcp:` block. Every model-supplied path resolves through symlinks and must land inside a declared directory, hidden (dot-prefixed) paths are unreachable outright (`.env` and `.kiri/` stay out of the transcript and the feed DB), and a failed config load empties the sandbox rather than widening it (see *Agentic sessions → Session filesystem access*).
